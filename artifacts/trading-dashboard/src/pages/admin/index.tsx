...
import {
  ...
  Loader2, Lock, RefreshCw, LogOut, BarChart2, Settings, Link2,
  LayoutDashboard,
} from "lucide-react";
...
function CopyButton({ text, label }: { text: string; label?: string }) {
  ...
  return (
    ...
    >
      ...
      {!label && (copied ? <span>Copied!</span> : null)}
    </button>
  );
}
...
export default function AdminPanel() {
  ...
  const subscriberLink = typeof window !== "undefined"
    ? window.location.origin + (import.meta.env.BASE_URL !== "/" ? import.meta.env.BASE_URL : "")
    : "";
            ...
            <div className="rounded-xl border border-border/40 bg-card/30 overflow-hidden">
              ...
              {keys.length === 0 ? (
                <div className="py-12 text-center text-muted-foreground text-sm">
                  No keys generated yet. Go to <button onClick={() => setTab("settings")} className="text-primary underline">Settings</button> to generate one.
                </div>
              ) : (
                    ...
                    return (
                      <div key={k.id} className={cn("px-5 py-3.5 flex items-center gap-4", !k.isActive && "opacity-50")}>
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          {k.isActive && !expired ? (
                            <button onClick={() => revoke(k.id)} title="Revoke" className="p-1.5 rounded hover:bg-amber-500/10 text-muted-foreground hover:text-amber-400 transition-colors"><Ban className="w-4 h-4" /></button>
                          ) : !k.isActive ? (
                            <button onClick={() => activate(k.id)} title="Re-activate" className="p-1.5 rounded hover:bg-emerald-500/10 text-muted-foreground hover:text-emerald-400 transition-colors"><CheckCircle2 className="w-4 h-4" /></button>
                          ) : null}
                          <button onClick={() => deleteKey(k.id)} title="Delete" className="p-1.5 rounded hover:bg-rose-500/10 text-muted-foreground hover:text-rose-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </div>
                    );
                  })}
        ...
        {tab === "settings" && (
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-border/40 bg-card/30 p-5 space-y-4">
              <h2 className="text-sm font-bold flex items-center gap-2">
                <Link2 className="w-4 h-4 text-primary" /> Subscriber Access Link
              </h2>
              <p className="text-xs text-muted-foreground">
                Send this link to your subscribers. They will see the access key entry screen.
              </p>
              ...
              <p className="text-[11px] text-muted-foreground/60 flex items-center gap-1">
                ...
                The admin panel at <code className="font-mono">/xk-manage</code> is never visible to subscribers.
              </p>
            </div>
            ...
            <div className="rounded-xl border border-border/40 bg-card/30 p-5 space-y-4">
              <h2 className="text-sm font-bold flex items-center gap-2">
                <Plus className="w-4 h-4 text-primary" /> Generate Access Key
              </h2>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="text" value={genLabel} onChange={e => setGenLabel(e.target.value)}
              ...
              {newKey && (
                <div className="rounded-lg bg-emerald-500/10 border border-emerald-500/30 p-4 space-y-2">
                  <p className="text-xs text-emerald-400 font-semibold">Key generated — copy and send to subscriber:</p>
                  ...
                  <div className="pt-1 border-t border-emerald-500/20">
                    <p className="text-[11px] text-muted-foreground mb-1">Share this message with your subscriber:</p>
                    <div className="flex items-start gap-2 bg-black/20 rounded px-3 py-2">
                      <p className="flex-1 text-xs text-foreground/70">
                        {"Your SmartFX access key: "}<strong>{newKey.key}</strong>{"\nActivate at: "}{subscriberLink}
                      </p>
                      <CopyButton
