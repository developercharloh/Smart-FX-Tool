/**
 * Transactions — EA trades (DB-persisted) + signal history
 */

import { useEffect, useState } from "react";
import {
  RefreshCw, TrendingUp, TrendingDown, Circle, CheckCircle2,
  XCircle, History, Activity, ShieldAlert, ShieldCheck, Zap,
} from "lucide-react";
...
export default function TradesPage() {
  ...
  async function fetchAll() {
    ...
    setError(null);
    try {
      const [tradesRes, histRes, haltRes] = await Promise.all([
        fetch(`${BASE}/api/ea/trades`),
        fetch(`${BASE}/api/signals/history?limit=200`),
        fetch(`${BASE}/api/ea/halt`),
      ]);
      if (tradesRes.ok) {
    ...
    <div className="p-6 space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-4 justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">EA live trades + all resolved signal history</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={halting}
            style={{
              background: halted ? "rgba(52,211,153,0.12)" : "rgba(248,113,113,0.12)",
              border:     halted ? "1px solid rgba(52,211,153,0.3)" : "1px solid rgba(248,113,113,0.3)",
            }}
          ...
          >
            {halting ? (
              ...
              <><ShieldCheck className="w-4 h-4 text-emerald-400" /><span className="text-emerald-300">Resume EA</span></>
            ) : (
              <><ShieldAlert className="w-4 h-4 text-rose-400" /><span className="text-rose-300">Halt EA</span></>
            )}
          </button>
      ...
      {halted && (
        <div style={{ background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.25)" }}
          ...
          <div>
            <p className="text-sm font-bold text-rose-300">EA is halted — no new trades will open</p>
            <p className="text-xs text-slate-500 mt-0.5">Click "Resume EA" above to restart trading. All settings are saved.</p>
          </div>
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-500">— positions opened by MT5 Expert Advisor (DB-persisted)</span>
        </div>
