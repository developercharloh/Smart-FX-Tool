//+------------------------------------------------------------------+
//|                                            SmartFX_EA.mq5        |
//|                       SmartFX AI Trading System v3.2             |
...
//  5. Experts log tab should show "SmartFX EA v3.2 started".
//
//  WHAT IT DOES
//  ────────────
//  • Every 10 s: fetches the latest high-confidence signal from the dashboard
//  • Dashboard signals are generated by multi-timeframe (W1→D1→H1→M15) AI analysis
//  • Signals are blocked during high-impact news windows (NFP, FOMC, CPI, ECB, BOE)
//  • Only fires when 5+ independent confluence factors agree (60%+ directional score)
...
//  CHANGELOG v3.2
//  ──────────────
//  • Compiled 0 errors / 0 warnings (all string params now pass by value)
//  • Deriv synthetic symbol table retained for broker symbol resolution only
//  • Dashboard now adds W1 top-down MTF filter, news calendar filter,
//    psychological level detection, and 8 candlestick patterns
//
//+------------------------------------------------------------------+
...
//── Broker symbol resolution table (synthetic display names) ───────
//   Retained for brokers that name pairs with Deriv display names.
string SYN_KEY[] = {
...
int OnInit()
{
   ...
   Print("SmartFX EA v3.2 started | Balance: ",
...
bool CheckDailyLimits()
{
   ...
   g_dailyStopped = true;
   Comment("SmartFX v3.2 | *** STOPPED — ", reason, " ***");
   return true;
}
...
//+------------------------------------------------------------------+
//| Symbol resolver                                                   |
//| Tries: plain → broker suffixes → synthetic display names         |
//+------------------------------------------------------------------+
string ResolveSymbol(const string &pair)
{
   ...
   // 2. Common broker suffixes (.m  +  _  pro  n  ECN  c)
   ...
   // 3. Synthetic display names (retained for broker compatibility)
...
//+------------------------------------------------------------------+
//| HTTP GET helper — pass URL by value (avoids MQL5 rvalue ref error)|
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| HTTP POST helper — pass both params by value                      |
//+------------------------------------------------------------------+
